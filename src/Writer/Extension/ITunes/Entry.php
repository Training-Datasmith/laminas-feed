...
class Entry
{
     ...
     */
    public function set_itunes_block($value): void
    {
     ...
     * @throws Writer\Exception\InvalidArgumentException
     */
    public function set_itunes_title($value): static
    {
        if ($this->string_wrapper->strlen($value) > 255) {
            throw new Writer\Exception\InvalidArgumentException('invalid parameter: "title" may only contain a maximum of 255 characters');
        }
     ...
     * @throws Writer\Exception\InvalidArgumentException
     */
    public function set_itunes_episode($number): static
    {
        if (!is_numeric($number) || is_float($number)) {
            throw new Writer\Exception\InvalidArgumentException(sprintf('invalid parameter: "number" may only be an integer; received %s', get_debug_type($number)));
        }
     ...
     * @throws Writer\Exception\InvalidArgumentException
     */
    public function set_itunes_episode_type($type): static
    {
        $valid_types = ['full', 'trailer', 'bonus'];
        if (!in_array($type, $valid_types, true)) {
            throw new Writer\Exception\InvalidArgumentException(sprintf('invalid parameter: "episodeType" MUST be one of the strings [%s]; received %s', implode(', ', $valid_types), get_debug_type($type)));
        }
     ...
     * @throws Writer\Exception\InvalidArgumentException
     */
    public function set_itunes_is_closed_captioned($status): static
    {
        if (!is_bool($status)) {
            throw new Writer\Exception\InvalidArgumentException(sprintf('invalid parameter: "isClosedCaptioned" MUST be a boolean; received %s', get_debug_type($status)));
        }
        if (!$status) {
            return $this;
        }
     ...
     * @throws Writer\Exception\InvalidArgumentException
     */
    public function set_itunes_season($number): static
    {
        if (!is_numeric($number) || is_float($number)) {
            throw new Writer\Exception\InvalidArgumentException(sprintf('invalid parameter: "season" may only be an integer; received %s', get_debug_type($number)));
        }
