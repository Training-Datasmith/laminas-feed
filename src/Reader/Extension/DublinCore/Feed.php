...
class Feed extends Extension\Abstract_Feed
{
    ...
    public function get_authors()
    {
        ...
        $authors = [];
        $list = $this->get_xpath()->query('//dc11:creator');
        if (!$list->length) {
            $list = $this->get_xpath()->query('//dc10:creator');
        }
        if (!$list->length) {
            $list = $this->get_xpath()->query('//dc11:publisher');
            if (!$list->length) {
                $list = $this->get_xpath()->query('//dc10:publisher');
            }
        }
        if ($list->length) {
    ...
    public function get_copyright()
    {
        ...
        $copyright = $this->get_xpath()->evaluate('string(' . $this->get_xpath_prefix() . '/dc11:rights)');
        if (!$copyright) {
            $copyright = $this->get_xpath()->evaluate('string(' . $this->get_xpath_prefix() . '/dc10:rights)');
        }
        if (!$copyright) {
            $copyright = null;
        }
    ...
    public function get_language()
    {
        ...
        $language = $this->get_xpath()->evaluate('string(' . $this->get_xpath_prefix() . '/dc11:language)');
        if (!$language) {
            $language = $this->get_xpath()->evaluate('string(' . $this->get_xpath_prefix() . '/dc10:language)');
        }
        if (!$language) {
            $language = null;
        }
